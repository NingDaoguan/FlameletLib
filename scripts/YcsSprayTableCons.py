...
for n in nLoop:
    ...
    if y == '1':
        ...
        filename2 = './tablesYc2/flameletTable_{:}.csv'.format(n)
    else:
        print('Input Error!')
    # Read names
    ...
    CO2Index = 13
    COIndex = 12
    Yc1 = data1[H2Index] + data1[COIndex]
    Yc2 = data1[H2OIndex] + data1[CO2Index] 
    YAR = data1[6]
    YARO = YAR[0]
    Z = (YAR - YARO) / (0.0-YARO)
    ...
    if y == '1':
        ...
        data2.append(list(Yc1))
        data2.append(list(Yc2))
        ax1.plot(Z,Yc1)
        ax1.plot(Z,Yc2)
        ax2.plot(Z,T)
    else:
